...
IrqResult* qmp_set_irq(uint32_t num, uint32_t level, Error **errp)
{
    IrqResult *irq_res = g_malloc0(sizeof(IrqResult));

    if(cur_sc->pending_intr == 0)
    {
        cur_sc->pending_intr = num;
        irq_res->queued = 1;
        return irq_res;
    }
...
void qmp_do_decision(uint64_t addr, uint8_t should_fork, Error **errp)
{
    cur_sc->should_fork = should_fork;
    cur_sc->cur_decision = addr;
    cur_sc->decision_ready = true;
}

LocationInfo* qmp_get_eip(Error **errp)
{
    uint64_t pc = sc_get_pc(); 
    LocationInfo *loc_info = g_malloc0(sizeof(LocationInfo));
    loc_info->addr = pc;
    return loc_info;
}

CallStackFrameList* qmp_get_callstack(uint64_t num_frames, Error **errp)
{
    return get_callstack(num_frames);
}


void qmp_sc_add_breakpoint(uint64_t addr, uint8_t type, Error **errp)
{
    ScBreakpoint bp = {addr, type}; 
    assert(cur_sc->active_bp_count < 16);
    cur_sc->breakpoints[cur_sc->active_bp_count++] = bp;
}
...
void qmp_finalize_io(uint64_t addr, uint32_t val, Error **errp)
{
    finalized_io fio = {.addr=addr,.val=val}; 
    assert (cur_sc->fio_idx < NumFinalize);
    cur_sc->finalized_io[cur_sc->fio_idx++] = fio;
}


void qmp_synth_options(uint32_t arch, Error **errp)
{
    cur_sc->archname = arch;
}
